...
class DilatedResnetBlock(nn.Module):
    ...
    def build_conv_block(self, dim_in, dim_out, dilation, padding_type, norm_layer, use_dropout, use_bias):
        # PADDING_TYPE not implemented yet
        conv_block = []
        conv_block += [nn.ReflectionPad2d(dilation)]
        conv_block += [nn.Conv2d(dim_in, dim_out, kernel_size=3, padding=0, bias=use_bias, dilation=dilation),
                       norm_layer(dim_out),
                       nn.ReLU(True)]
        if use_dropout:
            conv_block += [nn.Dropout(0.5)]
        ...
        conv_block += [nn.ReflectionPad2d(dilation)]
        conv_block += [nn.Conv2d(dim_out, dim_out, kernel_size=3, padding=0, bias=use_bias, dilation=dilation),
                       norm_layer(dim_out)]
    ...
    def forward(self, x):
        if hasattr(self, 'shortcut'):
            #print ( self.shortcut(x).size(), self.conv_block(x).size() )
...
class DilatedFCN(nn.Module):
    def __init__(self, C,
        ...
        if padding_type == 'reflect':
            ...
            # do the padding in the `padding` argument of conv2d
            model = []
        model = [nn.Conv2d(C,
                           C*mult[0],
                           kernel_size=7,
                           padding=0 if padding_type == 'reflect' else dilations[0],
                           bias=use_bias),
                 norm_layer(C*mult[0]),
                 nn.ReLU(True)]
